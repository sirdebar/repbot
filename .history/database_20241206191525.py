...
def add_user(tg_id: int, username: str):
    ...
    Если tg_id неизвестен, сохраняем только username.
    """
    if tg_id is None:
        cursor.execute("INSERT OR IGNORE INTO users (tg_id, username) VALUES (NULL, ?)", (username,))
    else:
        cursor.execute("INSERT OR IGNORE INTO users (tg_id, username) VALUES (?, ?)", (tg_id, username))
    conn.commit()
    logger.info(f"Пользователь {'NULL' if tg_id is None else tg_id} (@{username}) добавлен в базу.")
