...
async def search_user_handler(message: types.Message):
    ...
    if input_data.startswith("@"):
        username = input_data[1:]  # Убираем "@" для поиска
        user_info = get_user_by_username(username)

    # Если запрос состоит только из цифр, ищем по tg_id
    ...
    if user_info:
        ...
        # Убедимся, что username корректен
        username = username or f"user_{tg_id}"  # На случай отсутствия имени
    else:
        # Если пользователь не найден, создаем профиль
        ...
        reputation = 0  # Новым пользователям репутация равна 0
