...
from database import (
    ...
    update_related_tg_id, update_reviews_and_reputation
)
...
async def start_handler(message: types.Message):
    ...
    if existing_user:
        ...
        # Переносим отзывы и репутацию, если были оставлены до регистрации
        update_reviews_and_reputation(username, tg_id)

        logger.info(f"Профиль @{username} связан с tg_id {tg_id}.")
...
async def handle_review_input(message: types.Message, state: FSMContext):
    ...
    # Сохраняем отзыв с привязкой к username или tg_id
    add_review(target_id=target_id, username=username, changer_id=changer_id, review=review)

    await state.clear()
    if target_id:
        user = get_user_by_tg_id(target_id)
        username = user[1]
    await message.answer(bold(f"Отзыв успешно добавлен для @{username}."))
