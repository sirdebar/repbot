...
from database import (
    ...
    update_related_tg_id, add_review_by_username
)
...
async def search_user_handler(message: types.Message):
    ...
    if input_data.startswith("@"):
        ...
        if not user_info:
            # Создаем профиль с фиктивным tg_id
            temp_tg_id = hash(username) % (10**9)
            add_user(temp_tg_id, username)
            user_info = get_user_by_username(username)
...
async def reputation_handler(callback: types.CallbackQuery, state: FSMContext):
    ...
    target_username = user[1]  # username пользователя
    ...
    await state.update_data(target_username=target_username, action=action, changer_id=changer_id)
    await callback.message.edit_text(bold("Оставьте отзыв о пользователе (от 5 до 40 слов):"))


@router.message(ReviewStates.waiting_for_review)
async def handle_review_input(message: types.Message, state: FSMContext):
    data = await state.get_data()
    target_username = data["target_username"]  # Извлекаем username
    action = data["action"]
    ...
    if not user:
        # Создаем новый профиль с фиктивным tg_id
        temp_tg_id = hash(target_username) % (10**9)
        add_user(temp_tg_id, target_username)
        user = get_user_by_username(target_username)

    # Обновление репутации и сохранение отзыва
    target_id = user[0]  # tg_id пользователя
    try:
        add_review(target_id, changer_id, review)
        value = 1 if action == "add" else -1
        update_reputation(target_id, value)  # Обновляем репутацию через tg_id
        update_reputation_change_time(changer_id, target_id)
        await message.answer(bold(f"Отзыв о @{target_username} сохранен. Репутация обновлена."))
    except Exception as e:
        await message.answer(bold(f"Ошибка: {str(e)}"))
...
async def reset_reputation_handler(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(bold("Репутация сброшена администратором."))
