...
async def start_handler(message: types.Message, state: FSMContext):
    ...
    # Ищем пользователя сначала по tg_id
    user = get_user_by_tg_id(tg_id)

    if not user:
        # Если не нашли, пробуем искать по username
        user = get_user_by_username(username)

        if user:
            # Если нашли профиль по username, обновляем tg_id
            existing_tg_id, _, _, _ = user
            update_related_tg_id(existing_tg_id, tg_id)
        else:
            # Если профиль не найден ни по tg_id, ни по username, создаем новый
