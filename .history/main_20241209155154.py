...
from database import (
    ...
    update_related_tg_id, update_captcha_status, captcha_passed
)
import random
...
async def search_user_handler(message: types.Message):
    ...
    if user_info:
        ...
        reputation = 0
        captcha_passed = False  # Новый пользователь ещё не прошёл капчу

    # Проверка прохождения капчи
    if not captcha_passed:
        await message.answer("Пользователь не прошёл капчу. Выполните /start для активации профиля.")
        return
