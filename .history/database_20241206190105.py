...
def add_review(target_id: int = None, username: str = None, changer_id: int = None, review: str = ""):
    """
    Добавление отзыва.
    """
    if target_id is None and username is None:
        raise ValueError("Нужно указать target_id или username.")

    cursor.execute("""
        INSERT INTO reviews (target_id, username, changer_id, review)
        VALUES (?, ?, ?, ?)
    """, (target_id, username, changer_id, review))
    conn.commit()
    logger.info(f"Добавлен отзыв от {changer_id} для {target_id or username}.")

def update_reviews_and_reputation(username: str, tg_id: int):
    """
    Переносит отзывы и репутацию с username на tg_id.
    """
    # Перенос отзывов
    cursor.execute("""
        UPDATE reviews
    ...
    # Перенос изменений репутации
    cursor.execute("""
        UPDATE reputation_changes
        SET target_id = ?
        WHERE username = ?
    """, (tg_id, username))
    
    conn.commit()
    logger.info(f"Отзывы и изменения репутации перенесены с @{username} на tg_id {tg_id}.")
