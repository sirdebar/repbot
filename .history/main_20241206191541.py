...
async def search_user_handler(message: types.Message):
    ...
    if user_info:
        ...
        tg_id = None  # Пока не известно, оставляем None
        add_user(tg_id, username)  # Сохраняем только username
        reputation = 0
    ...
    if message.from_user.username == ADMIN_USERNAME:
        buttons.append([InlineKeyboardButton(text="Очистить репутацию", callback_data=f"reset_{tg_id}")])
