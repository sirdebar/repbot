...
from database import (
    ...
    can_change_reputation, update_reputation_change_time, add_user, sync_user_profile 
)
...
async def start_handler(message: types.Message):
    ...
    existing_user = get_user_by_tg_id(tg_id) or get_user_by_username(username)
    
    if existing_user:
        # Если профиль найден, обновляем информацию
...
async def search_user_handler(message: types.Message):
    ...
    if user_info:
        ...
        # Если пользователя нет в базе, пытаемся получить его данные через Telegram API
        try:
            ...
            add_user(tg_id, username)  # Создаем нового пользователя
            reputation = 0
        except Exception as e:
            # Если API не смогло найти пользователя, создаем фиктивный профиль
            logger.warning(f"Ошибка поиска: {e}. Создается фиктивный профиль.")
            tg_id = int(input_data) if input_data.isdigit() else hash(input_data) % (10**9)
