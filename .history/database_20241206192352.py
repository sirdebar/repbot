...
def add_review_by_username(target_username: str, changer_id: int, review: str):
    """
    Сохраняет отзыв для пользователя по username.
    Если пользователь с данным username не найден, создает нового.
    """
    user = get_user_by_username(target_username)
    
    if not user:
        # Создаем пользователя с временным tg_id
        temp_tg_id = hash(target_username) % (10**9)  # Генерация фиктивного tg_id
