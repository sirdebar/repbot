...
async def handle_review_input(message: types.Message, state: FSMContext):
    ...
    target_username = data["target_username"]  # Извлекаем username вместо ID
    action = data["action"]
    ...
    # Изменение репутации и сохранение отзыва
    try:
        add_review_by_username(target_username, changer_id, review)
        value = 1 if action == "add" else -1
        user = get_user_by_username(target_username)
        update_reputation(user[0], value)  # Обновляем репутацию через tg_id
        update_reputation_change_time(changer_id, user[0])
        await message.answer(bold(f"Отзыв о @{target_username} сохранен. Репутация обновлена."))
    except ValueError as e:
        await message.answer(bold(str(e)))
