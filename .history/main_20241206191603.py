...
async def start_handler(message: types.Message):
    ...
    # Проверяем, есть ли запись с таким username
    existing_user = get_user_by_username(username)

    if existing_user:
        existing_tg_id, _, reputation = existing_user
        if existing_tg_id is None:
            # Если запись существует, но tg_id не установлен, обновляем его
        ...
        elif existing_tg_id != tg_id:
            # Связываем все связанные данные, если tg_id уже есть
            update_related_tg_id(existing_tg_id, tg_id)

        await message.answer(
            bold(f"Добро пожаловать, {message.from_user.full_name}!\n"
                 f"Ваш профиль связан с репутацией: {reputation}."),
            reply_markup=main_menu
        )
    else:
        # Если пользователя с таким username или tg_id нет, создаем нового
...
async def search_user_handler(message: types.Message):
    ...
    if user_info:
        ...
        # Сохраняем профиль с username, tg_id пока неизвестен
        username = input_data[1:] if input_data.startswith("@") else f"user_{input_data}"
        add_user(None, username)
        reputation = 0

    # Запрет на взаимодействие с профилем админа
    if username == ADMIN_USERNAME:
        await message.answer(
            bold(f"Профиль администратора @{ADMIN_USERNAME}\n\nРепутация: ∞")
        )
        return

    # Кнопки взаимодействия с пользователем
    buttons = [
        [InlineKeyboardButton(text="🟢 +REP", callback_data=f"add_{tg_id or 'NULL'}"),
         InlineKeyboardButton(text="🔴 -REP", callback_data=f"sub_{tg_id or 'NULL'}")],
        [InlineKeyboardButton(text="Отзывы", callback_data=f"reviews_{tg_id or 'NULL'}")]
    ]

    # Кнопка для очистки репутации, если ищет админ
    if message.from_user.username == ADMIN_USERNAME:
        buttons.append([InlineKeyboardButton(text="Очистить репутацию", callback_data=f"reset_{tg_id or 'NULL'}")])
...
async def reset_reputation_handler(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(bold("Репутация пользователя сброшена."))
