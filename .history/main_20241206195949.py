...
async def start_handler(message: types.Message):
    ...
    if existing_user:
        ...
        # Привязываем отзывы, если пользователь создался новым
        update_reviews_target_id_from_username(username, tg_id)
        await message.answer(
            bold(f"Добро пожаловать, {message.from_user.full_name}!\nВаш профиль создан."),
            reply_markup=main_menu
...
async def handle_review_input(message: types.Message, state: FSMContext):
    ...
    if not target_id:
        ...
        if user:
            target_id = user[0]  # tg_id
            update_reviews_target_id_from_username(username, target_id)

    update_reputation(target_id, 1 if action == "add" else -1)
    add_review(target_id, changer_id, review, username=username)
    update_reputation_change_time(changer_id, target_id)

    await state.clear()
    user = get_user_by_tg_id(target_id) or {"username": username}
